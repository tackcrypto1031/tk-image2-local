...
export function createCodexRunner({
  ...
  setupLauncher = launchCodexCliSetupWindow,
...
export function buildCodexCliSetupScript({ codexBin = DEFAULT_CODEX_BIN } = {}) {
  const codexLiteral = quotePowerShellString(codexBin || DEFAULT_CODEX_BIN);
  return [
    "$ErrorActionPreference = 'Continue'",
    "$Host.UI.RawUI.WindowTitle = 'Codex CLI setup'",
    `$codexBin = ${codexLiteral}`,
    "function Write-Step([string]$Message) { Write-Host ''; Write-Host $Message -ForegroundColor Cyan }",
    "function Pause-And-Exit([int]$Code) { Write-Host ''; Read-Host '按 Enter 關閉此視窗'; exit $Code }",
    "Write-Host 'Codex CLI 安裝 / 登入檢查' -ForegroundColor Green",
    "Write-Host '此視窗由 Codex 圖像畫布開啟。完成後請回到工具左下角查看狀態。'",
    "Write-Step '檢查 Codex CLI...'",
    "$codexCommand = Get-Command $codexBin -ErrorAction SilentlyContinue",
    "if (-not $codexCommand) {",
    "  Write-Host '未偵測到 Codex CLI，準備使用 npm 安裝 @openai/codex@latest。' -ForegroundColor Yellow",
    "  $npmCommand = Get-Command npm -ErrorAction SilentlyContinue",
    "  if (-not $npmCommand) {",
    "    Write-Host '找不到 npm。請先安裝 Node.js，再重新點擊左下角 Codex CLI 狀態按鈕。' -ForegroundColor Red",
    "    Pause-And-Exit 1",
    "  }",
    "  Write-Step '安裝 Codex CLI...'",
    "  & npm install -g @openai/codex@latest",
    "  $installCode = if ($null -ne $LASTEXITCODE) { $LASTEXITCODE } else { 1 }",
    "  if ($installCode -ne 0) {",
    "    Write-Host \"Codex CLI 安裝失敗，結束碼：$installCode\" -ForegroundColor Red",
    "    Pause-And-Exit $installCode",
    "  }",
    "  $codexCommand = Get-Command $codexBin -ErrorAction SilentlyContinue",
    "  if (-not $codexCommand) {",
    "    $npmPrefix = (& npm prefix -g 2>$null | Select-Object -First 1)",
    "    if ($npmPrefix) {",
    "      $candidate = Join-Path $npmPrefix 'codex.cmd'",
    ...
    "  }",
    "  if (-not $codexCommand) {",
    "    Write-Host '已執行安裝，但目前視窗仍找不到 codex 指令。請重新開啟工具或手動確認 npm global PATH。' -ForegroundColor Red",
    "    Pause-And-Exit 1",
    "  }",
    "} else {",
    "  Write-Host \"已偵測到 Codex CLI：$($codexCommand.Source)\" -ForegroundColor Green",
    "}",
    "Write-Step '開啟 Codex 登入流程...'",
    "Write-Host '請依照 Codex CLI 指示完成 ChatGPT 登入。'",
    "& $codexBin login",
    "$loginCode = if ($null -ne $LASTEXITCODE) { $LASTEXITCODE } else { 0 }",
    "if ($loginCode -eq 0) {",
    "  Write-Host 'Codex 登入流程已結束。回到工具後，左下角狀態會自動更新。' -ForegroundColor Green",
    "} else {",
    "  Write-Host \"Codex login 結束碼：$loginCode\" -ForegroundColor Yellow",
    "}",
    "Pause-And-Exit $loginCode",
    ""
  ].join("\r\n");
}
